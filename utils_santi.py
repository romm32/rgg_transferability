...
def transform_matrix(adj_matrix, all = True):
    ...
    if all:
        ...
        # Abrir un archivo .txt en modo escritura ('w')
        with open('lista_numeros.txt', 'a') as archivo:
        # Escribir cada elemento de la lista en una nueva línea
            for elemento in lista_parejas:
...
def graphs_to_tensor(train=True, num_channels=5, num_features=1, b5g=False, building_id=990):
    ...
    band = ['2_4', '5']
    # path = '../graphs/' + 
    path = ''#str(band[b5g]) + '_' + str(building_id) + '/'
...
def get_rates_roms(phi, x, sigma, A):
    # phi: (batch_size, N, 1)
    # x:   (batch_size, N, 1)
    # A:   (batch_size, N, N)
    
    phi = phi.view(phi.size(0), -1, 1)   # (B, N, 1)
    x   = x.view(x.size(0), -1, 1)       # (B, N, 1)

    numerator = x * phi                  # (B, N, 1)
    denominator = torch.matmul(A.float(), phi.float()) + sigma  # (B, N, 1)
